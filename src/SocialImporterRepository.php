...
use OffbeatWP\SocialImporter\Channels\Instagram\InstagramChannel;
use OffbeatWP\SocialImporter\Channels\Facebook\FacebookChannel;
use OffbeatWP\SocialImporter\Channels\SocialPostEntity;
use OffbeatWP\SocialImporter\Models\SocialPostModel;

class SocialImporterRepository
{
    public function getRecent($count = 15, $type = null) {
        $socialPosts = SocialPostModel::where([]);        

        if ($type) {
            $socialPosts = $socialPosts->whereMeta('type', $type, 'IN');
        }

        $socialPosts = $socialPosts->take($count);
